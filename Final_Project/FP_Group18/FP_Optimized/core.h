...
public:
    explicit LocalSRAM(int cap = LOCAL_SRAM_FLOATS)
        : cap_(cap), mem_(cap, 0.f) {}

    void write(int offset, const std::vector<float>& v) {
        assert(offset + (int)v.size() <= cap_);
        std::memcpy(&mem_[offset], v.data(), v.size() * sizeof(float));
        write_count_ += (long long)v.size();
    }
    void write(int offset, const float* data, int n) {
        assert(offset + n <= cap_);
        std::memcpy(&mem_[offset], data, (size_t)n * sizeof(float));
        write_count_ += n;
    }

    std::vector<float> read(int offset, int n) {
        assert(offset + n <= cap_);
        std::vector<float> out(n);
        std::memcpy(out.data(), &mem_[offset], (size_t)n * sizeof(float));
        read_count_ += n;
        return out;
    }
...
//   FC6/FC7: 16 cores, 256 neurons/core  [OPT: was 8 cores]
//   FC8+Softmax: Core 15 only
//
// PE internal (Optimized):
...
SC_MODULE(Core) {
    ...
    // FC weight buffers (Weight Stationary)
    // FC6/FC7: 16 cores, 256 neurons/core  [OPT: was 512]
    // FC8: core 15 only
    std::vector<float> fc6_w, fc6_b;
    std::vector<float> fc7_w, fc7_b;
    std::vector<float> fc8_w, fc8_b;

    // Local SRAM per PE: partial sum buffer for OS dataflow  [OPT: new]
    // Capacity: LOCAL_SRAM_FLOATS (16 KB = 4096 floats)
    // Used to accumulate output FM partial sums during Conv computation.
    // Capacity changeable via LOCAL_SRAM_FLOATS in config.h.
    LocalSRAM local_sram_;
    ...
    // Per-core compute cycle accumulator
    long long total_compute_cycles_ = 0;
    ...
    void compute_thread() {
        while (true) {
            ...
            if (ptype == PKT_CONV1_IN && core_id >= 0 && core_id <= 3) {
                ...
                {
                    long long mac_ops = (long long)16 * 3 * 11*11 * 55*55;
                    long long cycles  = (mac_ops + MAC_PER_PE - 1) / MAC_PER_PE;
                    total_compute_cycles_ += cycles;
                    wait(cycles * CLK_PERIOD_NS, SC_NS);
                }
                std::vector<float> out;
                do_conv1(flat, out);
                // Conv1 output: 16*27*27 = 11664 floats > LOCAL_SRAM_FLOATS (4096)
                // Local SRAM is sized for Conv2-5 partial sums only; Conv1 bypasses it.
            ...
            // Conv2-5
                ...
                int Cin_arr[] ={0,0,64,192,384,256};
                int K_arr[]   ={0,0, 5,  3,  3,  3};
                int Hout_arr[]={0,0,13, 13, 13,  6};
                int Wout_arr[]={0,0,13, 13, 13,  6};
                int ch_per = ch_per_arr[layer];
                int Cin    = Cin_arr[layer];
                int K      = K_arr[layer];
                int Hout   = Hout_arr[layer];
                int Wout   = Wout_arr[layer];
                // [OPT] 16 MACs/PE
                {
                    long long mac_ops = (long long)ch_per * Cin * K*K * Hout * Wout;
                    long long cycles  = (mac_ops + MAC_PER_PE - 1) / MAC_PER_PE;
                    total_compute_cycles_ += cycles;
                    wait(cycles * CLK_PERIOD_NS, SC_NS);
                }
                ...
                // [OPT] Write partial sum to local SRAM then read back
                // out size <= 24*13*13 = 4056 <= LOCAL_SRAM_FLOATS(4096)
                int out_sz = (int)out.size();
                if (out_sz <= local_sram_.capacity()) {
                    local_sram_.write(0, out);
                    out = local_sram_.read(0, out_sz);
                }
                LOG2("[Core " << std::setw(2) << core_id
                    << "] Conv" << layer
            ...
            // FC weight preload
                ...
                int round = cs;
                int Nin   = (round==6)?9216:4096;
                // [OPT] CORES_FC6=16, so Nout = 4096/16 = 256
                int Nout  = (round==8)?1000:(4096/CORES_FC6);
                int wsz   = Nin*Nout;
                if (round==6) {
            ...
            // [OPT] CORES_FC6=16, all cores 0-15 participate
                ...
                int round = cs;
                // [OPT] Nout = 4096/16 = 256 neurons/core
                int Nout  = 4096/CORES_FC6;
                int Nin   = (round==6) ? 9216 : 4096;
                // [OPT] 16 MACs/PE
                {
                    long long mac_ops = (long long)Nout * Nin;
                    long long cycles  = (mac_ops + MAC_PER_PE - 1) / MAC_PER_PE;
                    total_compute_cycles_ += cycles;
                    wait(cycles * CLK_PERIOD_NS, SC_NS);
                }
            ...
            // FC8 + Softmax
                ...
                // [OPT] 16 MACs/PE
                {
                    long long mac_ops = (long long)1000 * 4096;
                    long long cycles  = (mac_ops + MAC_PER_PE - 1) / MAC_PER_PE;
                    total_compute_cycles_ += cycles;
                    wait(cycles * CLK_PERIOD_NS, SC_NS);
                }
