...
struct InputLayerImpl {
    FeatureMap output;
    InputLayerImpl()
        : output(3, std::vector<std::vector<float>>(227, std::vector<float>(227, 0.0f))) {}

    void load_data_and_pad(const std::vector<float>& raw) {
        for (auto& ch : output) for (auto& row : ch) std::fill(row.begin(), row.end(), 0.0f);
        for (int c = 0; c < 3; ++c)
            for (int r = 0; r < 224; ++r)
...
struct ConvLayerImpl {
    int in_channels=0, out_channels=0, kernel_size=0, stride=1, padding=0, in_h=0, in_w=0;
    ...
    void process(const FeatureMap& input) {
        ...
        int pad_h = in_h + 2*padding, pad_w = in_w + 2*padding;
        FeatureMap padded(in_channels, std::vector<std::vector<float>>(pad_h, std::vector<float>(pad_w, 0.0f)));
                    ...
                    for (int ic=0; ic<in_channels; ++ic) {
                        ...
                        for (int kh=0; kh<kernel_size; ++kh)
                            for (int kw=0; kw<kernel_size; ++kw)
                                sum += padded[ic][oh*stride+kh][ow*stride+kw]*weights[w_base+kh*kernel_size+kw];
                    }
...
struct MaxPoolLayerImpl {
    int channels=0, pool_size=0, stride=1, in_h=0, in_w=0;
    FeatureMap output;
...
struct SoftmaxLayerImpl {
    ...
    void process(const std::vector<float>& input) {
        ...
        float sum = 0.0f;
        for (int i=0; i<size; ++i) { output[i] = std::exp(input[i]-mx); sum += output[i]; }
